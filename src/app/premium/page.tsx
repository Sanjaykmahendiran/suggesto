...
export default function Premium() {
    ...
    const handleNotify = async () => {
        try {
            ...
            const response = await fetch(`https://suggesto.xyz/App/api.php?gofor=pronotify&user_id=${userId}`)
    ...
    return (
        // <PageTransitionWrapper>
            <div className="fixed max-w-sm mx-auto h-screen">
                {/* Header */}
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[1, 2].map((item) => (
                                    <div
                                        key={item}
                                        className="bg-gradient-to-br from-[#6c5ce7]/10 to-[#6c5ce7]/5 backdrop-blur-sm border border-[#6c5ce7]/20 rounded-xl p-3 text-center shadow flex flex-col items-center justify-start max-h-[180px] h-[180px] animate-pulse"
                                    >
                                        ...
                                        >
                                            <div className="grid grid-cols-2 gap-3">
                                                {chunk.map((feature, i) => (
                                                    <div
                                                        key={i}
                                                        className={`bg-gradient-to-br from-[#6c5ce7]/15 to-[#6c5ce7]/5  border border-[#6c5ce7]/20 rounded-xl p-3 text-center  flex flex-col items-center justify-start max-h-[180px] h-[180px] transform transition-all duration-300 ${!isDragging ? 'hover:scale-105' : ''}`}
                                                    >
                ...
                <div className="px-6 mb-6">
                    <div className="bg-gradient-to-br from-[#6c5ce7]/15 to-[#6c5ce7]/5 backdrop-blur-sm border border-[#6c5ce7]/20 rounded-full p-4 text-white flex items-center justify-between shadow-lg w-full max-w-md mx-auto">
                ...
                <div className="px-6 mb-4">
                    {!isChecked && showCheckError && (
                        <p className="text-red-500 text-sm mb-2 text-center">Please check the box to continue.</p>
                    )}
                    <Button
                    ...
                    >
                        Notify Me <ArrowRight className="w-6 h-6 ml-2" />
                    </Button>
                </div>
                ...
                {showSuccessMessage && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                        <div className="bg-white text-center px-6 py-4 rounded-2xl shadow-xl max-w-xs w-full">
                            <p className="text-primary font-semibold text-sm">Success!</p>
                            <p className="text-gray-700 text-sm mt-1">You'll be notified when premium is available.</p>
                        </div>
                    </div>
                )}
                ...
                {/* Error Toast */}
                {errorMessage && (
                    <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-red-600 text-white px-4 py-2 rounded-full shadow-lg z-50 transition-all duration-300">
                        {errorMessage}
                    </div>
                )}
            </div>
